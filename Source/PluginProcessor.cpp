...
tree (*this, nullptr, "PARAMETERS",
      {
          ...
          std::make_unique<AudioParameterFloat>("deacy","Decay",NormalisableRange<float>(1.0f, 2000.0f), 1.0f),
          ...
          std::make_unique<AudioParameterFloat>("filterType","FilterType",NormalisableRange<float>(0.0f, 2.0f), 1.0f),
...
{
    ignoreUnused(samplesPerBlock);//ignore unused samples from last key that we pressed
    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i))))
        {
            myVoice->getEnvelopeParams(tree.getRawParameterValue("attack"),
                                    tree.getRawParameterValue("deacy"),
